let app = new Vue({
    ...
    computed: {
      ...
      getCurrentTips(){
        return _tips= this.bills * this.getCurrentTipsPercents/100;
      },
      getTotalBills(){
         return +this.bills + this.getCurrentTips;
        },

      getSumBills(){
        let _sum = 0;
        this.listBill.forEach(item => {
        _sum = _sum + ((+item.percent * +item.bill/100) + +item.bill)
        });
        return _sum;
    ...
    methods: {
      onSubmitBill(){
        this.listBill.push({
          bill: this.bills,
          percent: this.getCurrentTipsPercents, 
          tip: this.getCurrentTips,  
        });
        this.bills = 0;
